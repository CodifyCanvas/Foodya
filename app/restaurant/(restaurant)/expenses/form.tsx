    ...
    try {
      ...
      if (!response.ok) {
        toast.error(result?.error ?? (isEditing
          ? "Expense can't be updated. Please try again."
          : "Expense can't be created. Please try again."))
        return
      }
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
            ...
            <DialogHeader className="p-6 pb-0">
              ...
              <DialogDescription className="sr-only">
                Create or Update your Expense Transaction
              </DialogDescription>
            </DialogHeader>

            {/* === Scrollable Form Area === */}
            <ScrollArea className="flex flex-col justify-between p-3">
              ...
              <div className="w-full py-2">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="group relative w-auto sm:max-w-sm m-1">
                      <FormLabel htmlFor="select_category_expense_form" className="bg-card text-muted-foreground absolute start-2 top-0 z-10 block -translate-y-1/2 px-1 text-xs">
                        Select Category
                      </FormLabel>
                      <FormControl>
                        <SelectInput options={categories}
                          ...
                          id="select_category_expense_form"
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
